...
function NoticeList() {
  ...
  const { search, searchParam, changeSearchInput, list, goAddPage, clear } = listStore;
  const { boardType, searchType, searchWord } = searchParam;

  const handleRowDoubleClick = (selectedInfo) => {
    navigate(`/notices/${selectedInfo.data.boardKey}`);
  };
              ...
              <form>
                <div className="form-inline justify-end">
                  ...
                  <AppButton
                    ...
                    onClick={search}
                  />
                  <AppButton style={{ marginLeft: 10 }} value="등록" onClick={goAddPage} />
                </div>
              </form>
            </div>
